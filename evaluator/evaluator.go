...
func Eval(node ast.Node, env *object.Environment) object.Object {
	...
	case *ast.LetStatement:
		...
		// need to associate the value with the identifier
...
func evalBangOperatorExpression(right object.Object) object.Object {
	...
	case NULL:
		return TRUE
	// this means that we consider everything else to be truthy
	// so for example `!0` would be `false`
	default:
		return FALSE
	}
}
...
func evalFloatInfixExpression(
	...
	case "-":
		return &object.Float{Value: leftVal - rightVal}
	case "*":
		// this currently lets us overflow float64 pretty easily
		return &object.Float{Value: leftVal * rightVal}
	case "/":
		return &object.Float{Value: leftVal / rightVal}
	case "<":
...
// extend the function env, not global env
// because
//
//	let adder = fn(x) { fn(y) { x + y } };
//	let addTwo = adder(2);
//
// we want to have the scope of adder in addTwo
// x is not bound in top level environment
// a closure is a closure because of the bindings it has access to when called
